...
public class UserProcessor {
        ...
        while (processor.doUntil(XMLEvent.START_ELEMENT, "User")) {
            ...
            if (cities.get(cityRef) == null) {
                ...
                if (!groups.keySet().containsAll(groupNames)) {
                    List<String> notExistsGroupInDB = groupNames.stream()
                                    .filter(a -> !groups.containsKey(a))
                                    .collect(Collectors.toList());

                    failed.add(new FailedEmails(xmlUser.getEmail(), "Groups '" + Joiner.on(", ").join(notExistsGroupInDB) + "' is not present in DB"));
                } else {
                    final User user = new User(id++, xmlUser.getValue(), xmlUser.getEmail(), UserFlag.valueOf(xmlUser.getFlag().value()), cityRef);

                    for (String group: groupNames) {
                        UserGroup userGroup = new UserGroup(user.getId(), groups.get(group).getId());
                        chunkUserGroups.add(userGroup);
                    }
    ...
    private void addChunkFutures(Map<String, Future<List<String>>> chunkFutures, List<User> chunk, List<UserGroup> chunkUserGroups) {
        ...
        Future<List<String>> future = executorService.submit(() -> {
            List<User> alreadyPresents = userDao.insertAndGetConflictEmails(chunk);
            Set<Integer> alreadyPresentsIDs = StreamEx.of( alreadyPresents).map(u -> u.getId()).toSet();

            userGroupDao.insertBatch(StreamEx.of(chunkUserGroups).filter(ug -> !alreadyPresentsIDs.contains(ug.getUserId())).toList());

            List <String> emails = StreamEx.of(alreadyPresents).map(user -> user.getEmail()).toList();
            return  emails;
        });
